...
void aggregate_maindet_AT_1_color(){
  ...
  for(int plt=0;plt<nDV-1;plt++){
    ...
    fGraph->GetListOfFunctions()->Add(ex2);
...
//for Ca 6%
//      both runs are with IHWP IN
//      if(run[plt]==1){
//for Pb/Diamond
//      if((run[plt]>=4110&&run[plt]<=4114)){
//for C 1%
//      if(run[plt]==4108){
//Correcting sign for IHWP OUT
      if((run[plt]>=4110&&run[plt]<=4114)||run[plt]==4108){
  ...
  c1->SaveAs(Form("%s.meanvsRun_color.pdf",dv_list[plt].Data()));
  }
  for(int plt=0;plt<nDV-1;plt++){
...
//Correcting sign for IHWP OUT
   ...
   for(int plt=0;plt<npt;plt++){
	...
	x_err[plt] = 0.0;
      ...
      eGraph->GetListOfFunctions()->Add(ex);
  ...
  c1->SaveAs(Form("%s.meanvsRun_AT1_color.pdf",plot_mean.Data()));
  }
...
//Correcting sign for IHWP OUT
  ...
  for(int plt=0;plt<npt;plt++){
    ...
    c1->SaveAs(Form("%s.meanvsRun_color_color.pdf",iv_list[iiv].Data()));
  }
  ...
  for(int plt=0;plt<npt;plt++){
    ...
    c1->SaveAs(Form("%s.rmsvsRun_color_color.pdf",iv_list[iiv].Data()));
  }

  for(Int_t idv=0;idv<nDV-1;idv++){
    for(Int_t iiv=0;iiv<nIV;iiv++){
      dv_name = dv_list[idv];
      iv_name = iv_list[iiv];
      dv_name.ReplaceAll("asym_","");
  ...
  for(int plt=0;plt<npt;plt++){
    ...
    c1->SaveAs(Form("%s_%svsRun_color_color.pdf",dv_name.Data(),iv_name.Data()));
  }
  }
