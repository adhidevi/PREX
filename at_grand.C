void at_grand(){
     ...
     agg->Add("/chafs2/work1/apar/aggRootfiles/slugRootfiles/grandRootfile/summary/rootfiles/grand_aggregator_1-59.root");
...
cout<<"wein:"<<wein_state[i]<<"\t"<<"ihwp:"<<ihwp_state[i]<<endl;
     ...
     for(int plt=0;plt<n_var;plt++){
       ...
       for(int i=0;i<npt;i++){
         ...
         gSystem->Exec("pdfunite *.pdf plots/aggretate_AT_blinded_26-54.pdf");
}
