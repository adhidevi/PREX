void at_grand_dit_main(){
     gStyle->SetOptFit(111);
     TChain *agg = new TChain("agg");
     agg->Add("/chafs2/work1/apar/aggRootfiles/slugRootfiles/dithering/grandRootfile/grand_60-86.root");
     ...
     for(int i=0;i<npt;i++)
         slug[i] = *(dummy+i);
     npt=agg->Draw("ihwp:Entry$",slug_cut,"goff");
     dummy = agg->GetV1();
     ...
     for(int plt=0;plt<n_var;plt++){
       ...
       for(int i=0;i<npt;i++){
         ...
         gSystem->Exec("pdfunite *.pdf plots/blindedAT/aggretate_AT_dit_blinded_82-86.pdf");
